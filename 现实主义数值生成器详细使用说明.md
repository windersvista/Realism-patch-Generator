...
- 输出文件命名规则：
	...
	- 当某输入文件内 `CURRENT_PATCH` 占比 > 50%：输出保持原文件名 `原文件名.json`
...
编辑 `weapon_refinement_rules.py`：
- `WEAPON_CALIBER_RULE_MODIFIERS`：口径二级修正
- `WEAPON_STOCK_RULE_MODIFIERS`：枪托形态二级修正
- `CALIBER_PROFILE_KEYWORDS`：口径识别关键词
...
说明：附件规则应用时会启用 `ensure_fields=True`，即规则字段缺失时会先补种子值，再做区间采样。

### 调整弹药规则

编辑 `ammo_rule_ranges.py`：
- `AMMO_PROFILE_RANGES`：第一层口径基础区间
- `AMMO_PENETRATION_TIERS` / `AMMO_PENETRATION_MODIFIERS`：第二层穿深分层
- `AMMO_SPECIAL_KEYWORDS` / `AMMO_SPECIAL_MODIFIERS`：第三层型号增量
...
- 若是瞄具分类异常，优先检查 `Name` 关键词是否能命中 `scope_red_dot` / `scope_magnified` 识别逻辑。
- 若是弹匣分类异常，检查输入里是否有容量字段（如 `Capacity`、`CartridgeMaxCount`），否则会回退到标准弹匣档。
